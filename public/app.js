...
function renderSettings(settings, { fillInputs }) {
  ...
  // This line used to restate the interval, the window and the backoff as well. The form
  // below holds those and the countdown above is already showing them, so what is left is
  // the one thing the rest of the page cannot say: the daily window does not apply to
  // watches that buy on their own. Said only when it applies, and only when the window is
  // a limit at all -- with the window off, "dygnet runt" is what everything already does.
  const aroundClock = settings.aroundTheClock && settings.activeFrom !== settings.activeTo;
  settingsStatus.textContent = aroundClock
    ? "Bevakningar som köper helt på egen hand kollas dygnet runt — de väcker ingen."
    : "";
  settingsStatus.hidden = !aroundClock;
