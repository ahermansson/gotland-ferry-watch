 ...
 * here, straight after this returned ok. `npm run book` (below) calls prepareBooking and
 * then closes it unpressed, for testing the flow with no purchase involved at all.
 */
...
export async function prepareBooking(watch: Watch): Promise<PreparedBooking | FailedBooking> {
  // Always headless. This runs unattended, on a schedule, at hours nobody is watching --
  // a visible window and a 50 ms slowMo were there to be followed by eye during an
  // approval, and there is no approval left to follow. The screenshot in the Discord
  // receipt is what a purchase is inspected through now.
 ...
 * calls it, from exactly one place: buyNow, straight after prepareBooking returned ok.
 * Never from the scraper, never from a route, and never from `npm run book`, whatever the
 * settings say.
 */
